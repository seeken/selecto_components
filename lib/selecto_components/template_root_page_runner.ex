defmodule SelectoComponents.TemplateRootPageRunner do
  @moduledoc false

  alias Phoenix.Component
  alias SelectoComponents.{TemplateInstance, TemplateRootCursor, TemplateSourceExecutor}
  import Phoenix.LiveView, only: [connected?: 1, start_async: 3]

  @running_assign :template_running_root_pages
  @default_timeout_ms 15_000
  @default_max_concurrent 4

  def initialize(socket), do: Component.assign(socket, @running_assign, %{})

  @spec cursor(Phoenix.LiveView.Socket.t(), binary(), map(), binary(), keyword()) ::
          {:ok, map()} | {:error, map()}
  def cursor(socket, source_id, scope, secret, opts \\ []) do
    with {:ok, manifest, snapshot} <- TemplateInstance.runtime(socket),
         {:ok, source} <- source_plan(manifest, source_id) do
      TemplateRootCursor.issue(snapshot, source_id, source, scope, secret, opts)
    end
  end
  ...
  def start(socket, source_id, token, authorize, secret, opts \\ []) do
    with true <- connected?(socket),
         ...
         {:ok, source_state, after_values} <- ready_source(snapshot, source_id),
      ...
      key =
        {:selecto_template_root_page, source_id, source_state["generation"], source_state["page"],
         make_ref()}
      ...
      executor_opts =
        ...
        |> Keyword.drop([
          ...
          :root_secret,
          :source_timeout_ms,
          :max_concurrent_root_pages
        ])
        |> Keyword.merge(root_snapshot: snapshot, root_cursor: token, root_secret: secret)
      ...
      identity = %{
        ...
        "expected_after_values" => after_values
      }

      socket =
        socket
        |> start_async(key, fn ->
          execute_page(manifest, effect, authorize, executor_opts, identity, timeout_ms)
        end)
      ...
      _ ->
        TemplateInstance.assign_error(
          socket,
          TemplateInstance.diagnostic("invalid_root_page_request", "root page request is invalid")
        )
    end
  end

  def handle_async({:selecto_template_root_page, _, _, _, _} = key, {:ok, {:ok, commit}}, socket) do
    socket = clear_running(socket, key)

    case TemplateInstance.commit_root_page(socket, commit) do
  ...
  def handle_async(
        {:selecto_template_root_page, _, _, _, _} = key,
        {:ok, {:error, error}},
        socket
      ) do
  ...
  def handle_async({:selecto_template_root_page, _, _, _, _} = key, {:exit, _reason}, socket) do
    error = TemplateInstance.diagnostic("root_page_task_failed", "root page task failed")
    {:noreply, elem(TemplateInstance.assign_error(clear_running(socket, key), error), 2)}
  end
  ...
  defp execute_page(manifest, effect, authorize, opts, identity, timeout_ms) do
    ...
    case Task.yield(task, timeout_ms) do
      ...
      nil ->
        ...
        {:error, TemplateInstance.diagnostic("root_page_timeout", "root page timed out")}
  ...
  defp ready_source(snapshot, source_id) do
    case get_in(snapshot, ["sources", source_id]) do
      %{
        ...
        "page" => page,
        "result" => %{"root_page" => %{"has_more" => true, "after_values" => values}}
      } = source
      when is_integer(generation) and generation > 0 and is_integer(page) and page > 0 and
             is_list(values) ->
        {:ok, source, values}

      _ ->
        {:error, TemplateInstance.diagnostic("source_not_ready", "root source is not ready")}
    end
  end

  defp limits(opts) when is_list(opts) do
    timeout = Keyword.get(opts, :source_timeout_ms, @default_timeout_ms)
    concurrent = Keyword.get(opts, :max_concurrent_root_pages, @default_max_concurrent)
